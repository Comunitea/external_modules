...
class StockMoveLine(models.Model):
    ...
    def get_use_create_lots(self):
        ...
        return picking_type_id and picking_type_id.use_create_lots or self.move_id.inventory_id or False
    ...
    def write(self, vals):
        if len(self) == 1 and vals.get('serial_ids_string', False):
            serial_names, warning = self.product_id.sanitize_lot_names(vals['serial_ids_string'])
    ...
    def filter_affected_moves(self):
        if self._context.get("by_pass_tracking", False) or 'inventory_mode' in self._context:
            return False
        # Los movimientos afectados son
        return self.filtered(lambda x: 
                                x.state != 'done' 
                                and not x.move_id.should_bypass_tracking()
                            )

    def _action_done(self):
        # Todoso los serial_virtual deben convertirse a serial antes de llegar aquí

        # Convierto serial_ids_string a nuemeros de serie, siempre que vengan
        
        moves = self.mapped('move_id')
        if self:
            affected_moves = self.filter_affected_moves()
            if affected_moves:
                self.filtered(lambda x: x.serial_ids_string).convert_serial_ids_string_to_serial_ids()
                # compruebo 
                ...
                if picking_type_id:
                    ...
                    # Si no tiene albarán vamos al caso más restrictivo ??? Despues reviso si hace falta cambiarlo
                    bypass_serial_error_location = False
                    check_serial_qties = True

                for sml_id in affected_moves:
                    if not sml_id.serial_ids and sml_id.qty_done > 0:
                        raise ValidationError(_('Serial numbers are required'))
        ...
        moves.mapped('move_line_ids').filtered(lambda x: x.serial_ids).write_serial_location()
        return res

    def convert_serial_ids_string_to_serial_ids(self):
        # esta función convierte el campo serial_ids_string a serial_ids
        for sml_id in self:
            lot_names, warning = sml_id.product_id.sanitize_lot_names(sml_id.serial_ids_string)
            if not lot_names:
                continue
            ...
            # Actualizo la cantidad hecha 
            if not self._context.get("by_pass_compute_qties", False):
                qty_done = len(sml_id.serial_ids)
                if sml_id.qty_done != qty_done:

...
let StockLocationListPageModule = class StockLocationListPageModule {
};
...
let StockLocationListPage = class StockLocationListPage {
    ...
    location_list_infinite_scroll_add() {
        ...
        this.stock.get_location_list(this.current_selected_type, this.offset, this.limit, this.search).then((data) => {
            let current_length = Object.keys(this.locations).length;
            if (Object.keys(data).length < 25) {
                this.limit_reached = true;
            }
            for (var k in data)
                this.locations[current_length + Number(k)] = data[k];
        })
            .catch((error) => {
            this.presentAlert('Error al recuperador el listado de operaciones:', error);
        });
...
StockLocationListPage.ctorParameters = () => [
...
//# sourceMappingURL=pages-stock-location-list-stock-location-list-module-es2015.js.map
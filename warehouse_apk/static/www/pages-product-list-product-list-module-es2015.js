...
let ProductListInfoComponent = class ProductListInfoComponent {
    constructor(router) {
        this.router = router;
    }
    ngOnInit() { }
...
ProductListInfoComponent.ctorParameters = () => [
...
let ProductListPage = class ProductListPage {
    constructor(odoo, router, storage, alertCtrl, audio, stock) {
        this.odoo = odoo;
        this.router = router;
    ...
    get_product_list(search = null) {
        this.offset = 0;
        this.limit_reached = false;
        this.stock.get_product_list(this.offset, this.limit, search).then((data) => {
    ...
    product_list_infinite_scroll_add() {
        ...
        this.stock.get_product_list(this.offset, this.limit, this.search).then((data) => {
            let current_length = Object.keys(this.product_list).length;
            if (Object.keys(data).length < 25) {
                this.limit_reached = true;
            }
            for (var k in data)
                this.product_list[current_length + Number(k)] = data[k];
        })
            .catch((error) => {
            this.presentAlert('Error al recuperador el listado de operaciones:', error);
        });
...
//# sourceMappingURL=pages-product-list-product-list-module-es2015.js.map
...
let LocationProductListComponent = class LocationProductListComponent {
    constructor(router) {
        this.router = router;
    }
    ngOnInit() { }
...
LocationProductListComponent.ctorParameters = () => [
...
let StockLocationProductListPageModule = class StockLocationProductListPageModule {
};
...
let StockLocationProductListPage = class StockLocationProductListPage {
    ...
    get_location_products(search = null) {
        this.offset = 0;
        this.limit_reached = false;
        this.stock.get_location_products(this.location, this.offset, this.limit, search).then((products_lists) => {
    ...
    product_list_infinite_scroll_add() {
        ...
        this.stock.get_location_products(this.location, this.offset, this.limit, this.search).then((data) => {
            let current_length = Object.keys(this.products).length;
            if (Object.keys(data).length < 25) {
                this.limit_reached = true;
            }
            for (var k in data)
                this.products[current_length + Number(k)] = data[k];
        })
            .catch((error) => {
            this.presentAlert('Error al recuperador el listado de productos:', error);
        });
...
StockLocationProductListPage.ctorParameters = () => [
...
//# sourceMappingURL=pages-stock-location-product-list-stock-location-product-list-module-es2015.js.map
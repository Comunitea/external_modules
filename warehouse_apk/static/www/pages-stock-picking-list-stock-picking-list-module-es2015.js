...
let PickingListComponent = class PickingListComponent {
    constructor(router) {
        this.router = router;
    }
    ngOnInit() { }
...
PickingListComponent.ctorParameters = () => [
...
let StockPickingListPageModule = class StockPickingListPageModule {
};
...
let StockPickingListPage = class StockPickingListPage {
    constructor(odoo, router, route, alertCtrl, audio, stock, storage) {
        this.odoo = odoo;
        this.router = router;
        this.route = route;
    ...
    get_picking_list(search = null) {
        this.offset = 0;
        this.limit_reached = false;
        this.stock.get_picking_list(this.view_domain[this.view_selector], this.current_type_id, this.offset, this.limit, search).then((picking_list) => {
    ...
    picking_list_infinite_scroll_add() {
        ...
        this.stock.get_picking_list(this.view_domain[this.view_selector], this.current_type_id, this.offset, this.limit, this.search).then((data) => {
            let current_length = Object.keys(this.pickings).length;
            if (Object.keys(data).length < 25) {
                this.limit_reached = true;
            }
            for (var k in data)
                this.pickings[current_length + Number(k)] = data[k];
        })
            .catch((error) => {
            this.presentAlert('Error al recuperador el listado de operaciones:', error);
        });
...
StockPickingListPage.ctorParameters = () => [
...
//# sourceMappingURL=pages-stock-picking-list-stock-picking-list-module-es2015.js.map
...
class StockPackingTypeList(models.Model):
    ...
    packing_type_id = fields.Many2one(
        'stock.packing.type', string="Tipo de empaquetado")
...
class StockPackOperationSscc(models.Model):
    ...
    packing_list_id = fields.Many2one('stock.packing.list', 'Packing')
...
class StockPackingList(models.Model):
    ...
    packing_type_id = fields.Many2one(
        'stock.packing.type', "Packing list type")
    packing_type_list_id = fields.Many2one(
        'stock.packing.type.list', "Packing type")
    picking_id = fields.Many2one('stock.picking', 'Parent picking')
    stock_move_ids = fields.One2many(
        'stock.move', 'packing_list_id', string='Contents')
    stock_move_line_ids = fields.One2many(
        'stock.move.line', 'packing_list_id', string='Contents')
    sscc_id = fields.Many2one('stock.move.line.sscc', 'SSCC')

...
class SignenModel(models.AbstractModel):
    ...
    def get_report(self, signen_report):
        if signen_report.report_type == "report":
            ...
            report = safe_eval(
                signen_report.execute_code,
                globals_dict={"obj": self, "signen_report": signen_report},
            )
        ...
        if report.report_type in ["qweb-html", "qweb-pdf"]:
            result, format = (
                self.env["report"].get_pdf([self.id], report_service),
                "pdf",
            )
        else:
            result, format = odoo_report.render_report(
                ...
                [self.id],
                report_service,
                {"model": signen_report.model_id.model},
                self._context,
            )
        if not report_name:
    ...
    def send_to_signen_job(self):
        ...
        )  # Solo funciona si hay 1 documento configurado por modelo.
        api_key = self.env["ir.config_parameter"].get_param("signen.api.key")
        user = self.env["signen.configuration.user"].search(
            [], limit=1
        )  # TODO: Tal vez quieran mas de 1 user por compañía?
        with Signen(user.username, user.password, api_key) as signen:
            ...
            self.write(
                {"signen_document_id": document_id, "signen_uploaded": True}
            )

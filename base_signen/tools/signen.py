...
class Signen(object):
    ...
    def __exit__(self, type, value, traceback):
        if not self.__nologin:
            self.__jwt = self.make_request(
                {},
                'user/logout',
                'GET')

    def make_request(self, data, url_model,
                     method, add_headers={}, files=None):
    ...
    def upload_file(self, filename, file_data, receivers_data):
        ...
        response = self.make_request(
            None, 'document/file', 'POST', files=files)
    ...
    def file_evidences(self, file_id):
        ...
        response = self.make_request(
            None, 'document/%s/file' % file_id, 'GET',
    ...
    def get_signature_status(self, file_id):
        ...
        for receiver in json.loads(response.content)['receivers']:
            sign_timestamp = receiver['signatures']['1'] and \
                receiver['signatures']['1'][0]['timestamp'] or False
    ...
    def create_user(self, username, password):
        response = self.make_request(
            ...
            'user', 'POST', add_headers={'Accept': 'application/pdf'})
        if response.status_code != 200:
            raise Exception(
                'Error creating user: {} - {}' %
                response.status_code, response.content)

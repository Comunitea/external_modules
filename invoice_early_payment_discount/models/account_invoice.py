        ...
        if prod_early_payment:
            ...
            for early_payment_line in early_payments:
                ...
                for invoice_line in self.env['account.invoice.line'].browse(
                    ...
                    epd_invoice_account = invoice_line.product_id.categ_id.\
                        property_account_early_payment_disc
                    if epd_invoice_account and \
                            str(epd_invoice_account.id) not in \
                            group_account_line[early_payment_line]:
                        group_account_line[early_payment_line][
                            str(epd_invoice_account.id)] = [invoice_line.id]
                    elif epd_invoice_account and str(epd_invoice_account.id) in \
                            group_account_line[early_payment_line]:
                        group_account_line[early_payment_line][
                        ...
                        if prod_income_account:
                            ...
                            group_account_line[early_payment_line][
                                str(epd_invoice_account.id)].append(
                                    invoice_line.id)
                    else:
                        raise UserError(
                ...
                new_lines += self.env['account.invoice.line'].with_context(
                    partner_id=partner_id).create({
                        ...
                        'account_id':
                            ...
                            property_account_early_payment_disc.id,
                        'price_unit': self.currency_id.round(
                            0.0 - (
                                self.compute_early_payment_discount(
                                    inv_lines_out_vat,

...
def intersect(la, lb):
    ...
    lf = [x for x in la if x in lb]
    return len(lf) == len(la) == len(lb)
        ...
        if prod_early_payment_id:
            for invoice in self:
                ...
                if not found:
                    ...
                    invoice.early_payment_discount_amount = float(
                        total_net_price) - \
                        (float(total_net_price) *
                         (1 - (invoice.early_payment_discount or 0.0) / 100.0))

    def compute_early_payment_discount(self, invoice_line_ids,
                                       early_payment_percentage):
        """computes early payment price_unit"""
        total_net_price = 0.0

        for invoice_line in self.env['account.invoice.line'].browse(
                invoice_line_ids):
            if invoice_line.product_id and \
                    invoice_line.product_id.without_early_payment:
                continue
        ...
        return float(total_net_price) - \
            (float(total_net_price) *
             (1 - (float(early_payment_percentage) or 0.0) / 100.0))
    ...
    def compute_early_payment_lines(self):
        ...
        if prod_early_payment:
            ...
            for early_payment_line in early_payments:
                ...
                for invoice_line in self.env['account.invoice.line'].browse(
                        early_payments[early_payment_line]):
                    epd_sale_account = invoice_line.product_id.categ_id.\
                        property_account_sale_early_payment_disc
                    if epd_sale_account and \
                            str(epd_sale_account.id) not in \
                            group_account_line[early_payment_line]:
                        group_account_line[early_payment_line][
                            str(epd_sale_account.id)] = [invoice_line.id]
                    elif epd_sale_account and str(epd_sale_account.id) in \
                            group_account_line[early_payment_line]:
                        group_account_line[early_payment_line][
                            str(epd_sale_account.id)].append(invoice_line.id)
                        ...
                        group_account_line[early_payment_line][str(
                            ...
                            epd_sale_account.id and \
                            str(epd_sale_account.id) in \
                            group_account_line[early_payment_line]:
                        if prod_income_account:
                            ...
                            group_account_line[early_payment_line][
                                str(epd_sale_account.id)].append(
                                    invoice_line.id)
                    else:
                        raise UserError(
                            _('Cannot set early payment discount because now \
is impossible find the early payment account. Review invoice products \
categories have defined early payment account by default or early payment \
discount product have defined an output account.'))

            partner_id = self.partner_id and self.partner_id.id or False
            for early_payment_line in group_account_line:
                for account_id in group_account_line[early_payment_line]:
                    new_lines += self.env['account.invoice.line'].with_context(
                        partner_id=partner_id).create({
                            ...
                            [(6, 0,
                             [int(x) for x in early_payment_line.split(',')])],
            ...
            if inv_lines_out_vat:
                new_lines += self.env['account.invoice.line'].with_context(
                    partner_id=partner_id).create({
                        ...
                        'product_id': prod_early_payment.id,
                        'account_id':
                            prod_early_payment.property_account_income_id.id or
                            prod_early_payment.categ_id and
                            prod_early_payment.categ_id.
                            property_account_sale_early_payment_disc.id,
                        'price_unit': self.currency_id.round(
                            0.0 - (
                                self.compute_early_payment_discount(
                                    inv_lines_out_vat,
    ...
    def button_compute_early_payment_disc(self):
        for invoice in self:
            if invoice.early_payment_discount:
                ...
                early_payment = invoice.env['product.product'].search(
                    [('default_code', '=', 'DPP')])
    ...
    def _onchange_partner_id(self):
        ...
        if not self.early_payment_discount:
            if not self.payment_term_id:
                ...
                early_discs = commercial_partner.early_payment_discount_ids.\
                    filtered(lambda x: x.payment_term_id ==

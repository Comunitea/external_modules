...
class AccountInvoice(models.Model):
    ...
    value_date = fields.Date(string='Fecha valor',
                             states={'draft': [('readonly', False)]},
                             index=True, copy=False)
    payment_ref_date = fields.Date(string='Fecha referencia pagos',
                             states={'draft': [('readonly', False)]},
                             index=True, copy=False)

    @api.onchange('payment_term_id', 'date_invoice', 'value_date')
    def _onchange_payment_term_date_invoice(self):
        if self.value_date and self.payment_term_id.reference_date == \
                'value_date':

...
class MoveChangeReserveLine(models.TransientModel):
    ...
    def get_picking_domain(self, move_id, quant):
        domain = [('default_location_src_id', 'child_of', move_id.location_id.id),
                  ('code', '=', move_id.picking_type_id.code),
                  ('default_location_dest_id', '=', move_id.location_dest_id.id),
                  ('warehouse_id', 'in', (False, move_id.picking_type_id.warehouse_id.id))]
        return domain

    def get_first_type(self, type_ids):
    ...
    def do_reserve(self):
        ...
        if move_id.location_id != quant.location_id:
            new_picking_type_id_ids = self.env['stock.picking.type'].search(self.get_picking_domain(move_id, quant), limit=1)
            new_picking_type_id = self.get_first_type(new_picking_type_id_ids)

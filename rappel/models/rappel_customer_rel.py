...
class ResPartnerRappelRel(models.Model):
    ...
    def _get_next_period(self):
        #TOCHECK: Uso de fechas como objetos
        self.ensure_one()
        if self.last_settlement_date and \
                self.last_settlement_date > self.date_start:
            date_start = self.last_settlement_date + relativedelta(days=1)
        else:
        ...
        date_stop = date_start + \
            relativedelta(months=self.PERIODICITIES_MONTHS[self.periodicity],
                          days=-1)
        if self.date_end and self.date_end < date_stop:
    ...
    def _get_invoices(self, period, products):
        ...
        invoices = self.env['account.move'].search(
            [('move_type', '=', 'out_invoice'),
             ('invoice_date', '>=', period[0]),
             ('invoice_date', '<=', period[1]),
             ('state', 'in', ['posted']),
        ...
        refund_lines = self.env['account.move.line'].search(
            [('move_id', 'in', [x.id for x in refunds]),
             ('product_id', 'in', products),
             ('no_rappel', '=', False)])
        invoice_lines = self.env['account.move.line'].search(
            [('move_id', 'in', [x.id for x in invoices]),
             ('product_id', 'in', products),
             ('no_rappel', '=', False)])
            ...
            if rappel.rappel_id.calc_mode == 'fixed':
                if rappel.rappel_id.calc_amount == 'qty':
                    ...
                    total = sum([x.price_subtotal
                        ...
                        abs(sum([x.price_subtotal for x in refund_lines]))
                    if total:
                ...
                total = sum([x[field] for x in invoice_lines]) - \
                    abs(sum([x[field] for x in refund_lines]))
                rappel_info["curr_qty"] = total
                if total:

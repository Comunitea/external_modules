...
class RappelAdvices(models.Model):
    ...
    ADVICE_TIMING = [('fixed', 'Days to finish'), ('variable', '% Period')]
...
class RappelCurrentInfo(models.Model):
    ...
    def _compute_display_name(self):
        for record in self:
            record.display_name = "%s %s - %s" % (record.partner_id.name,
                                                   record.date_start,
                                                   record.date_end)
        ...
        for partner in partner_pool:
            ...
            if pool_partners:
                ...
                for rappel in pool_partners:
                    date_end = rappel.date_end
                    date_start = rappel.date_start
                ...
                if values.get(partner.id):
                    ...
                    ctx.update({
                        ...
                        'mail_from': self.env.company.email,
